...
pub fn run_random_simulation(
    ...
    for _ in 0..num_points {
        ...
        // NOTE: Caclulate radius, using mass as a value for scaling.  This has no bearing on
        // calucation of mass to mass gravity, and only used for rendering purposes.
        let mass = rand::random::<f32>() * max_mass;
        let radius = 5e-1 * mass.powf(1.0 / 3.0);

        population.add_colour(rerun::Color::from_u32(rand::random::<u32>()));
        population.add_radius(radius);
    }
    ...
    population.add_colour(rerun::Color::from_u32(0));
    population.add_radius(600.);

    let mut population2 = population.clone();
    rr.set_time_nanos("stable_time", time.get_time() as i64);

    write_log(&mut rr, &mut population, &render_mode);
